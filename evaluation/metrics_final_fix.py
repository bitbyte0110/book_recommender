...
class RecommendationEvaluator:
    ...
    def evaluate_content_based_filtering(self, ratings_df, content_sim_matrix, top_n=10, threshold=3.0, test_size=0.2):
        """
        Evaluate content-based filtering system with FINAL FIX for MSE calculation
        Uses user's average rating as baseline and adjusts based on similarity
        """
        ...
        for user_id in users[:200]:  # Sample for efficiency
            ...
            # FINAL FIX: Use user's average rating as baseline and adjust based on similarity
            user_mse_scores = []
            for _, row in user_test_ratings.iterrows():
                ...
                if book_id in books_df['book_id'].values:
                    ...
                    # Use user's average rating as baseline, adjust based on similarity
                    # High similarity -> predict higher than average
                    # Low similarity -> predict lower than average
...
if __name__ == "__main__":
    # Test the final fixed content-based evaluation
    ...
    print("FINAL FIXED Content-Based Results:")
    ...
    print(f"\nRMSE Analysis:")
    print(f"Original RMSE: 3.1742")
    print(f"Fixed RMSE: {result['rmse']:.4f}")
    print(f"RMSE Improvement: {((3.1742 - result['rmse']) / 3.1742 * 100):.1f}%")
